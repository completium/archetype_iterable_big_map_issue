...
export const a_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.pair_array_to_mich_type([
        ...
        att.prim_annot_to_mich_type("bytes", ["%value"])
    ], ["%values"]), ["%values"]),
...
export class Bad {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/bad.arl", {}, params)).address;
        this.address = address;
    ...
    async get_a_value(key: att.Nat): Promise<att.Bytes | undefined> {
        if (this.address != undefined) {
            ...
            const raw_data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair)?.args[0]).toString()), key.to_mich(), a_key_mich_type);
